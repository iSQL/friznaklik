...
import { UserRole, Vendor, Service, Appointment, User as PrismaUser, AppointmentStatus, VendorStatus } from '@prisma/client';
import { redirect } from 'next/navigation';
import Link from 'next/link';
import { ShieldAlert, Store, UserCircle, ListOrdered, CalendarDays, Edit3, ArrowLeft, Phone, MapPin, Info, CheckCircle, XCircle, Clock } from 'lucide-react';
      ...
      <div className="container mx-auto py-8 px-4 md:px-6 text-center">
        <div className="card bg-base-200 shadow-xl max-w-md w-full mx-auto">
          <div className="card-body items-center text-center">
            ...
            <p className="mb-6">Nemate dozvolu za pregled detalja o salonima.</p>
    ...
    <div className="container mx-auto py-6 px-2 sm:px-4">
        ...
        <div className="card-body">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
            ...
            <div>
              ...
              <p><strong>Kreiran:</strong> {format(new Date(vendorDetails.createdAt), 'dd.MM.yyyy HH:mm', { locale: sr })}</p>
            </div>
          </div>
            ...
            <>
              ...
              <div className="overflow-x-auto">
                <table className="table table-sm w-full">
                  <tbody>
                    {Object.entries(vendorDetails.operatingHours as Record<string, {open: string, close: string}>).map(([day, hours]) => (
                      <tr key={day}>
                        <td className="font-semibold capitalize">{day.charAt(0).toUpperCase() + day.slice(1)}:</td>
                        <td>{hours.open ? `${hours.open} - ${hours.close}` : <span className="italic text-gray-500">Zatvoreno</span>}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
      ...
      <div className="card bg-base-100 shadow-xl mb-8">
        <div className="card-body">
          ...
          <p className="text-xs"><strong>Prisma User ID:</strong> <span className="font-mono">{vendorDetails.owner.id}</span></p>
          <p className="text-xs"><strong>Clerk User ID:</strong> <span className="font-mono">{vendorDetails.owner.clerkId}</span></p>
        </div>
      </div>
                ...
                <tbody>
                  {vendorDetails.appointments.map(app => (
                    <tr key={app.id} className="hover">
                      <td>{format(parseISO(app.startTime.toString()), 'dd.MM.yyyy HH:mm', { locale: sr })}</td>
