...
import type { AppointmentWithDetails } from './AppointmentList'; // Import the shared type
...
export default function AdminAppointmentCard({ appointment }: AdminAppointmentCardProps) {
  ...
  // Ensure startTime and endTime are parsed correctly if they are strings
    ...
    try {
      const response = await fetch(`/api/admin/appointments/${appointment.id}/duration`, {
        ...
        headers: {
          ...
          // Clerk's session token is typically handled automatically by its SDK for client-side fetches
          // to your own backend if middleware is set up correctly.
        },
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.message || `Failed to update duration (Status: ${response.status})`);
      }
  ...
  const handleUpdateStatus = async (newStatus: 'approved' | 'rejected') => {
    setIsUpdating(true); // Use the same loading state or a new one
    setError(null);
    setSuccessMessage(null);
    ...
    try {
        const response = await fetch(endpoint, {
            ...
            // No body needed for these specific PUT requests based on typical design
        });

        if (!response.ok) {
            const errorData = await response.json();
            throw new Error(errorData.message || `Failed to ${newStatus} appointment.`);
        }
        ...
        setError(err.message || `Could not ${newStatus} the appointment.`);
    } finally {
        setIsUpdating(false);
    }
};
      ...
      <div className="card-body">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-3">
            ...
            <span className={`badge badge-lg mt-2 sm:mt-0 ${
                ...
                appointment.status === 'cancelled' ? 'badge-neutral' : // Or another color for cancelled
                'badge-ghost'
            }`}>
        ...
        <div className="mt-4 pt-4 border-t border-base-300">
          ...
          <div className="flex flex-col sm:flex-row items-end gap-3">
            ...
            >
              {isUpdating ? <span className="loading loading-spinner loading-xs"></span> : 'Update Duration'}
            </button>
          </div>
        ...
        {appointment.status === 'pending' && (
            <div className="card-actions justify-end mt-6 pt-4 border-t border-base-300">
                <button 
                    ...
                    disabled={isUpdating}
                >
                    {isUpdating ? <span className="loading loading-spinner loading-xs"></span> : 'Approve'}
                </button>
                <button 
                    ...
                    disabled={isUpdating}
                >
                    {isUpdating ? <span className="loading loading-spinner loading-xs"></span> : 'Reject'}
                </button>
            </div>
        )}
