// src/components/admin/workers/AdminWorkersClient.tsx
'use client';
...
interface WorkerFormData {
  ...
  userClerkId: string | null; // Clerk ID of an existing User to link
}
...
const WorkerForm = ({
  ...
  useEffect(() => {
    if (initialData) {
      setFormData({
        ...
        userClerkId: initialData.userId || null, // Assuming initialData.userId IS the clerkId if fetched with user relation
                                                // If not, this needs adjustment based on how linked user data is fetched
      });
    } else {
  ...
  const handleSubmit = (e: FormEvent) => {
    ...
    if (!formData.name.trim()) {
        // Basic client-side validation, more robust on backend
    ...
    <dialog open className="modal modal-bottom sm:modal-middle modal-open">
      <div className="modal-box w-11/12 max-w-lg">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            <label htmlFor="userClerkId" className="label">
             ...
             <div className="input-group">
                ...
                className="input input-bordered w-full"
                disabled={isProcessing || !!initialData?.userId} // Disable if editing and already linked
                />
            </div>
            {initialData?.userId && (
...
// WorkerList Component (minor styling updates)
const WorkerList = ({
  workers,
  onEdit,
  onDeleteRequest, // Renamed to avoid confusion with actual delete handler
  isProcessingDelete,
  processingWorkerId,
}: {
    ...
    return (
      <div className="text-center py-10 card bg-base-200 shadow-md border border-base-300/50">
        <div className="card-body items-center">
          ...
          <p className="text-base-content/70 text-lg">Trenutno nema dodatih radnika.</p>
          <p className="text-sm text-base-content/50">Možete dodati prvog radnika koristeći dugme iznad.</p>
        </div>
      </div>
    );
  }
...
export default function AdminWorkersClient({ userRole, ownedVendorId }: AdminWorkersClientProps) {
  const [workers, setWorkers] = useState<WorkerWithOptionalUser[]>([]);
  const [isLoading, setIsLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [formActionError, setFormActionError] = useState<string | null>(null); // Error specific to form actions
  const [deleteActionError, setDeleteActionError] = useState<string | null>(null); // Error specific to delete action
  ...
  const fetchWorkers = useCallback(async () => {
    ...
    try {
      ...
      // Fetch user details if worker.userId exists
      const data: PrismaWorker[] = await response.json();
      const workersWithUsers = await Promise.all(data.map(async (worker) => {
        if (worker.userId) {
          // This is a simplified approach. In a real app, you might want a dedicated API endpoint
          // to fetch user details by Clerk ID or Prisma User ID for security and efficiency.
          // For now, we assume worker.userId is the Prisma User ID.
          // If worker.userId is ClerkId, you'd fetch from /api/admin/users?clerkId=...
          try {
            // This is a placeholder. You'd need an API to get user by Prisma ID or Clerk ID
            // const userRes = await fetch(`/api/admin/users/by-prisma-id/${worker.userId}`);
            // if (userRes.ok) {
            //   const userData = await userRes.json();
            //   return { ...worker, user: userData };
            // }
            // For now, let's assume no direct user fetching here to keep it simple,
            // or that the backend API for workers already includes this if needed.
            // If the worker API returns user details, this map is not needed.
          } catch (userFetchError) {
            console.error("Error fetching user for worker:", worker.userId, userFetchError);
          }
        }
        return worker as WorkerWithOptionalUser;
      }));
      setWorkers(workersWithUsers);
    } catch (err) {
  ...
  const handleSubmitForm = async (formData: WorkerFormData) => {
    ...
    const url = editingWorker
      ? `/api/admin/vendors/${ownedVendorId}/workers/${editingWorker.id}`
      : `/api/admin/vendors/${ownedVendorId}/workers`;
  ...
  const confirmDeleteWorker = async () => {
    ...
    setDeleteActionError(null);
    try {
      const response = await fetch(`/api/admin/vendors/${ownedVendorId}/workers/${workerToDelete.id}`, {
        method: 'DELETE',
      });
      const responseData = await response.json();
      if (!response.ok) {
  ...
  if (isLoading) {
    return (
      <div className="flex justify-center items-center h-[calc(100vh-250px)]">
        ...
        <p className="ml-4 text-lg text-base-content/70">Učitavanje radnika...</p>
      </div>
    );
  }
  ...
  return (
    <div>
      <div className="flex flex-col sm:flex-row justify-end items-center mb-6 gap-4">
        <button onClick={() => handleOpenForm()} className="btn btn-primary w-full sm:w-auto">
          <PlusCircle className="mr-2 h-5 w-5" /> Dodaj Novog Radnika
        </button>
      </div>
